...
/**
 * Super Sniffer: Deep multi-layered on-chain security and rug-defense engine
 */
export function evaluateTokenSafety(token: Partial<TokenData>, config: BotConfig): SafetyReport {
  ...
  // 7. Dev Wallet Allocation (Tightened to <= 5%)
  if (devHoldingPct > 5) {
  ...
  // 8. Wash Trading & Synthetic Volume Detection
  ...
  const isApproved = normalizedScore >= config.minSafetyScore && (freezeRevoked || !config.requireFreezeRevoked) && taxSafe;

  return {
    score: normalizedScore,
    isApproved,
    reasons: reasons.length > 0 ? reasons : ['All Super Sniffer checks passed (Revoked mint, disabled freeze, verified LP, low insider ownership)'],
    flags: {
      mintRevoked,
      freezeRevoked,
